app.lazy.controller('AdminRoles', function($rootScope, $scope, $routeParams, $http, $q, config, Data, Auth){
	$scope.temp = {};
	var Users = Data({
		rootUrl: 	'https://api.parse.com/1/',
		className: 	'users',
		fireRef: 	false
	});
	var Roles = Data({
	...
	var tools = $scope.tools = {
		...
		init: function(){
			Roles.tools.list().then(function(roles){
				$scope.roles = roles;
			})
			Users.tools.list().then(function(users){
				$scope.users = users;
			})
			$scope.$on(Roles.listener, function(e, roles) {
				$scope.roles = roles;
			});
			$scope.$on(Users.listener, function(e, users) {
				$scope.users = users;
			});
		},
		roles: {
			...
			create: function(name, roleArr, userArr){
				...
				Roles.tools.save(roleData).then(function(response){
			...
			set: function(role){
				...
				$http.get(config.parse.root+'/users?where={"$relatedTo":{"object":{"__type":"Pointer","className":"_Role","objectId":"'+role.objectId+'"},"key":"users"}}').success(function(data){
					...
					for(var i=0; i<data.results.length; i++)
						Users.tools.byId(data.results[i].objectId).then(function(user){
			...
			save: function(role){
				...
				for(var i=0; i<role.addRelation.length; i++)
					arelations.push(Users.tools.ref(role.addRelation[i]))
				for(var i=0; i<role.removeRelation.length; i++)
					rrelations.push(Users.tools.ref(role.removeRelation[i]))
				...
				if(arelations.length > 0){
					...
					Roles.tools.save(ar).then(function(results){
						Roles.tools.save(rr).then(function(results){
							console.log(results);
